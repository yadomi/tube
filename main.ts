...
function cronHandlerUpdateQueue() {
    ...
    for (const channel of channels) {
        ...
        logger.info(`[cronHandlerUpdateQueue] Channel "${channel[0]}" to queue, will be fetched at ${new Date(Date.now() + d).toISOString()}`);
    }
...
async function queueHandlerRSSFetcher({ channel_id }: { channel_id: string }) {
    ...
    try {
        ...
        for (const video of videos) {
            ...
            logger.info(`[queueHandlerRSSFetcher] Added video "${video.title} (${video['yt:videoId']})" / "${data.feed.title}" (${channel_id})`);
        }
    } catch (e) {
